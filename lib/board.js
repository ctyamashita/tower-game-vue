  ...
  methods: {
    ...
    keyUpHandler(e) {
      ...
      if (currentTile.type == 'tree') {
        setTimeout(() => this.hpUp(2, currentTile), 1000);
      } else if (currentTile.type == "portal") {
        this.disabledKeys = true;
        setTimeout(() => this.climbFloor(), 4500);
      }
    },
    keyDownHandler(e) {
    ...
    hpUp(amount, currentTile) {
      if (currentTile.hpUp && this.player.hp + amount <= this.player.max_hp) this.player.hp += amount;
    ...
    combatTurn() {
      ...
      setTimeout(() => this.playerCheck(), 600);
    },
    playerCheck() {
      this.applyDamage(this.player, this.enemy)
      if (this.player.isDead) return this.showResult("Game Over");
      setTimeout(() => this.endTurnAnimationReset(), 400);
    },
    endTurnAnimationReset() {
      const playerImg = document.querySelector('#player-model img');
      const enemyImg = document.querySelector('#enemy-model img');
      playerImg.classList = '';
      enemyImg.classList = '';
