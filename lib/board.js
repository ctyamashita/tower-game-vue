...
createApp({
  data() {
    ...
    return {
      ...
      player: {
        ...
        skillDescription: "200% ATK (can't crit)",
        passiveTitle: "Last Standing",
        passiveDescription: "Every 3 battles won, +1 to max HP"
      },
      enemy: {
        job: "warrior",
        hp: 9,
  ...
  methods: {
    ...
    loadAudio() {
      ...
      this.bgm.onpause = () => {
        if (this.audioOn) this.battleBGM.play()
      }
      this.bgm.onplay = () => {
        if (this.audioOn) {
    ...
    enableAudio(e) {
      ...
      this.audioOn && this.bgm.paused ? this.bgm.play() : this.bgm.pause()
    },
    ...
    // map generation
    initializeGame() {
      this.player = {...this.classes.find(obj=>obj.job == this.player.job)}
      this.resetBoard();
    },
    renderBoard() {
      setTimeout(() => {
    ...
    addEnemies(enemies) {
      ...
      enemies.forEach((enemy) => {
        ...
        enemy.max_hp = Math.floor(enemy.attributes.def / 3) + enemy.max_hp
    ...
    newGame() {
      ...
      const availableClasses = this.classes.sort((a,b)=> a.id - b.id)
      Swal.fire({
        title: 'Kyuuko Tower Arena',
        html: `
          <img src="images/icons/android-chrome-192x192.png" />
          <label class="hard-mode-btn" >Hard Mode
          <input type="checkbox" name="falling-blocks"/>
          </label>
          `,
        confirmButtonText: '<i class="fa-regular fa-circle-play"></i>',
        allowOutsideClick: () => {
          return false
        },
        didOpen: () => {
          const popUp = Swal.getPopup()
          const hardModeInput = popUp.querySelector('input[type=checkbox]');
          hardModeInput.addEventListener('change', (e) => {
            this.hardMode = e.currentTarget.checked
            this.player.hp = this.hardMode ? Math.round(this.player.max_hp / 2) : this.player.max_hp
          })
        }
      }).then(()=>{
        Swal.fire({
          ...
          html: `
            <div class="swiper mySwiper">
              <div class="swiper-wrapper">
                ${availableClasses.map(jobObj=>{
                  return `<div class="swiper-slide" id="${jobObj.job}">
                            <div class="inner-card">
                              <div class="card-back">
                                <div>
                                  <p>${jobObj.actions[1]}</p>
                                  <p>(Active)</p>
                                  <small>${jobObj.skillDescription}</small>
                                  <hr>
                                  <p>${jobObj.passiveTitle}</p>
                                  <p>(Passive)</p>
                                  <small>${jobObj.passiveDescription}</small>
                                </div>
                              </div>
                              <div class="card-front" style="background-image: url('images/${jobObj.job}-banner.png'), linear-gradient(-15deg, burlywood 25%, rgba(255,255,255,.5) 70%);">
                                <p class="job-name">${jobObj.job}</p>
                                <img src="images/${jobObj.job}.png" />
                                <div class="class-details">
                                  <p>${jobObj.shortDescription}</p>
                                </div>
                              </div>
                            </div>
                          </div>
          ...
          didOpen: () => {
            ...
            const swiper = new Swiper(".mySwiper", {
              ...
              on: {
                slideChange: (e) => {
                  const card = e.slides[e.activeIndex]
                  const classTemplate = {...this.classes.find(obj=>obj.job == card.id)}
                  this.player = {
                    lvl: 1,
                    xp: 0,
                    points: 0,
                  ...
                  if (this.hardMode) this.player.hp = Math.round(this.player.max_hp / 2)

                  this.baseHp = classTemplate.max_hp
                }
              }
            });
          },
    ...
    resetBoard() {
      ...
      this.ogAttributes = {...this.player.attributes}
      this.updateClock();
      this.skillPoints = 3;
    },
    // map actions
    move(keyPressed) {
      ...
      if (keyPressed == 'e') {
        ...
        if (this.isWalkable(newPosition)) {
          ...
          if (this.player.job == "necromancer") {
            if (this.time.light == 'night') {
              if (!this.buffed) {
                this.buffed = true
                Object.keys(this.player.attributes).forEach(attr=>{
                  this.player.attributes[attr] = Math.round(this.ogAttributes[attr] * 1.2)
                })
              }
            } else {
              this.buffed = false
              Object.keys(this.player.attributes).forEach(attr=>{
                this.player.attributes = {...this.ogAttributes}
              })
            }
          } else if (this.player.job == 'monk') {
            if (this.time.movementCount % 3 == 0) this.player.hp = this.player.hp + 1 >= this.player.max_hp ? this.player.max_hp : this.player.hp + 1 
          }
          ...
          if (this.isSpot(newPosition)) {
            ...
            setTimeout((scope) => {
              ...
              if (currentTile.hpUp && scope.player.hp < scope.player.max_hp) {
                let recoveredHp
                if (this.player.job == 'beastmaster') {
                  recoveredHp = Math.round(scope.player.max_hp * .2)
                } else {
                  recoveredHp = Math.round(scope.player.max_hp * .1)
                }
                scope.player.hp = (scope.player.hp + recoveredHp) >= scope.player.max_hp ? scope.player.max_hp : scope.player.hp + recoveredHp
              }
          ...
          if (this.hardMode) {
            ...
            if (currentTile.type == 'high-box') {
              const newTile = new Tile(x,y,'box')
              newTile.height = this.tiles[`${x}-${y}`].height
              this.tiles[`${x}-${y}`] = newTile
            } else if (currentTile.height == 'low') {
              this.tiles[`${x}-${y}`] = new Tile(x,y,'water')
            } else {
              currentTile.height = 'low'
            }
          }
        }
      }
    ...
    // combat
    combatTurn(action) {
      if (!this.currentCombat || this.currentCombat.ended || !this.inBattle) this.currentCombat = new Combat(this.player, this.enemy, {...this.player.attributes}, this.time.light);
      const animations = this.currentCombat.turn(action);
      this.turnCount = this.currentCombat.turnCount;
      this.skillTurnCount = this.currentCombat.skillTurnCount;
    ...
    endTurnAnimationReset(escape, action) {
      ...
      if (this.skillPoints > 0) document.querySelector('#skill-btn').removeAttribute('disabled')
        ...
        setTimeout(() => {
          ...
          if (gameOver) {
            ...
            Swal.fire({
              title: message,
              iconHtml: loseIcon,
              confirmButtonText: `<span><i class="fa-solid fa-arrow-rotate-left"></i></span>`,
              customClass: {
            ...
            Swal.fire({
              ...
              if (this.player.job === 'warrior') {
                if (this.battleCount % 3 == 0) this.player.max_hp++
              }
            })
          }
    ...
    xpUp(amount) {
      if (this.player.xp + amount >= 100) {
        ...
        Swal.fire({
          title: 'Level Up!',
          text: this.player.job == 'cleric' ? "+5 Attribute points" : "+3 Attribute points",
        })
      } else {
    ...
    updateAttribute(attr) {
      if (this.player.lvl == 1) this.baseHp = this.player.max_hp
      const plusButtons = document.querySelectorAll('.plus-btn');
      if (this.player.points > 0) {
        plusButtons.forEach(btn=>btn.removeAttribute('disabled'))
        this.player.attributes[attr]++
        if (attr == 'def') {
          this.player.max_hp = Math.floor(this.player.attributes.def/3) + this.baseHp
          this.player.hp = this.player.max_hp
        }
        this.player.points--
        this.ogAttributes = {...this.player.attributes}
    ...
    startCombat(coords) {
      if (this.audioOn && !this.inBattle) {
        this.bgm.pause()
      }
      this.enemy = this.tiles[this.stringifyCoords(coords)].enemy
      document.querySelector('.modal').classList.add('expand');
      if (this.skillPoints <= 0) document.querySelector('.modal #skill-btn').setAttribute('disabled', '')
      this.inBattle = true;
    },
    endCombat() {
      this.disabledKeys = false
      ...
      document.querySelector('#atk-btn').removeAttribute('disabled')
      document.querySelector('#skill-btn').removeAttribute('disabled')
      document.querySelector('#run-btn').removeAttribute('disabled')
      ...
      this.turnCount = 1;
      this.skillTurnCount = 0;
      if (this.player.job !== 'necromancer' && this.time.light !== 'night') {
        this.player.attributes = {...this.ogAttributes}
      }
    }
  }
