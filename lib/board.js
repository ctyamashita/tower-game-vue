...
createApp({
  ...
  methods: {
    ...
    tileContent(coords) {
      ...
      if (tile.type == 'spot') content.push(`<button class="${this.tiles[this.stringifyCoords(coords)].type}" id="${this.tiles[this.stringifyCoords(coords)].enemy?.id}"><i class="fas ${this.tiles[this.stringifyCoords(coords)].enemy?.icon}"></i></button>`)
