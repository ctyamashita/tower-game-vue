...
if ('serviceWorker' in navigator) {
  const path = location.origin == 'http://localhost:8000' ? 'service-worker.js' : '/tower-game-vue/service-worker.js'
  navigator.serviceWorker.register(path)
    .catch(function(err) { console.info('Service Worker not supported.') })
}
  ...
  methods: {
    ...
    renderBoard() {
      ...
      try {
        ...
        fetch(path)
          ...
          .then((json) => {
            setTimeout((scope) => {
              if (scope.audioOn) new Audio('./audio/Spawn.mp3').play()
            }, 1600, this);
            this.classes = json;
            this.generateTiles();
      ...
      if (keyPressed == 'e') {
        ...
        if (this.isWalkable(newPosition)) {
          ...
          if (this.isSpot(newPosition)) {
            ...
            if (currentTile.hpUp && this.audioOn) new Audio('./audio/HpUp.mp3').play()
            setTimeout((scope) => {
            ...
            if (this.audioOn) {
              const teleportSound = new Audio('./audio/Portal.mp3')
              teleportSound.playbackRate = .5
              teleportSound.play()
            }
            setTimeout((scope) => {
              scope.renderBoard();
    ...
    combatTurn(action) {
      ...
      if (this.audioOn) {
        if (animations.enemy.includes('critical')) {
          new Audio('./audio/HitCritical.mp3').play()
    ...
    xpUp(amount) {
      if (this.player.xp + amount >= 100) {
        ...
        new Audio('./audio/LvlUp.mp3').play()
        Swal.fire({
          title: 'Level Up!',
    ...
    endCombat() {
      if (this.audioOn) {
        ...
        if (!this.battleBGM.paused) this.battleBGM.pause()
      }
      document.querySelector('#atk-btn').removeAttribute('disabled')
      document.querySelector('#run-btn').removeAttribute('disabled')
