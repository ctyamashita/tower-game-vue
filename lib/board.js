...
createApp({
  ...
  methods: {
    ...
    tileContent(coords) {
      ...
      if (tile.type == 'spot') content.push(`<button class="${this.tiles[this.stringifyCoords(coords)].type}" id="${this.tiles[this.stringifyCoords(coords)].enemy?.id}"><i class="fa-solid fa-question"></i></button>`)
    ...
    spawnPlayer() {
      ...
      for (const [key, value] of Object.entries(this.tiles)) {
        if (value.walkable && value && value.type !== 'tree') spawnableTiles.push(key);
      }

      this.player.position = this.parseCoords(spawnableTiles.sample());
    },
    addEnemies(enemies) {
