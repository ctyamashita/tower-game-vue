...
export default class Combat {
  constructor(player,enemy, playerOgAttributes) {
    this.player = player
    this.playerOgAttributes = playerOgAttributes
    this.enemy = enemy
    this.turnCount = 1
  ...
  turn(action) {
    ...
    if (this.skillTurnCount > 0) {
      Object.keys(this.player.attributes).forEach(attr=>{
        this.player.attributes[attr] = this.playerOgAttributes[attr] + 2
      })
    } else {
      this.player.attributes = this.playerOgAttributes;
    }

    console.log(Object.values(this.playerOgAttributes) == Object.values(this.player.attributes))
    console.log(Object.values(this.playerOgAttributes), Object.values(this.player.attributes))
    this.turnCount += 1;
