...
export default class Combat {
  constructor(player,enemy, ogAttributes, time) {
    this.player = player
  ...
  turn(action) {
    ...
    // console.log(this.ogAttributes)

    // console.log(Object.values(this.ogAttributes) == Object.values(this.player.attributes))
    // console.log(Object.values(this.ogAttributes), Object.values(this.player.attributes))
    this.turnCount += 1;
    let playerCrit = this.criticalHit(this.player);
    let enemyCrit = this.criticalHit(this.enemy);
    let escape = this.runConfirmed(this.player, this.enemy);
    if (this.player.job == 'bard') escape = true;
    ...
    switch (action) {
      ...
      case 'skill':
        playerHit = this.hitConfirmed(this.enemy, this.player);
        if (['ranger', 'cleric', 'bard', 'monk', 'necromancer'].includes(this.player.job)) {
          // can't miss skills
    ...
    if (action === 'skill' && ['cleric', 'bard', 'necromancer'].includes(this.player.job)) {
      switch (this.player.job) {
        case 'cleric':
        case 'bard':
        case 'necromancer':
      ...
      if (action === 'run' && escape) {
        ...
        if (enemyHit) {
          if (['monk','necromancer'].includes(this.player.job) && action === 'skill') {
            ...
            // necromancer
            if (this.buffed && this.player.job == 'necromancer') {
              this.calculateDamage(this.player, this.enemy, 999, false)
            } else {
              // all other classes
    ...
    if ((this.enemy.isDead || this.player.isDead) || (this.buffed && this.skillTurnCount <= 0) || (action === 'run' && escape)) {
      Object.keys(this.player.attributes).forEach(attr=>{
        this.player.attributes[attr] = this.ogAttributes[attr]
      })
      this.buffed = false
    } else if (this.buffed) {
      Object.keys(this.player.attributes).forEach(attr=>{
  ...
  hitConfirmed(defender, attacker) {
    if (defender.job == 'ninja' && this.time == 'night') {
      return rand(attacker.attributes.dex) >= Math.floor(defender.attributes.agi/3) * 2
    } else {
