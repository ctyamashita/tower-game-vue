...
export default class Combat {
  ...
  turn(action) {
    if (this.enemy.isDead || this.player.isDead) return;
      ...
      if (action === 'run' && !escape) enemyAnimation.push('run-failed')
      // enemy phase
      let enemyHit
  ...
  hitConfirmed(defender, attacker) {
    return rand(defender.attributes.agi) <= Math.round(attacker.attributes.dex/3) + 3
  }

  runConfirmed(defender, attacker) {
    return rand(defender.attributes.agi) > Math.round(attacker.attributes.agi/3) + 3
  }
  ...
  calculateDamage(defender, attacker) {
    const damage = Math.round(attacker.attributes.atk - (defender.attributes.def/3 + 3));
