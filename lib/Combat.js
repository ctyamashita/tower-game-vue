...
export default class Combat {
        ...
        case 'skill':
          ...
          if (['ranger', 'cleric', 'bard', 'monk'].includes(this.player.job)) {
            switch (this.player.job) {
              case 'ranger':
                this.calculateDamage(this.enemy, this.player, 1.5)
                break;
              case 'cleric':
                this.player.hp += 3
                playerHit = false
            ...
            switch (this.player.job) {
              case 'warrior':
                ...
                break;
              case 'ninja':
                const atkModifier = rand(100) <= 30 ? 3 : 1
                this.calculateDamage(this.enemy, this.player, false, atkModifier)
                break;
              case 'beastmaster':
  ...
  calculateDamage(defender, attacker, def_modifier = false, atk_modifier = false) {
    const damage = Math.round((atk_modifier ? attacker.attributes.atk * atk_modifier : attacker.attributes.atk) - (def_modifier ? defender.attributes.def/3 * def_modifier : defender.attributes.def/3));
